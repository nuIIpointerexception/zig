...
pub fn toStringZ(p: Path, allocator: Allocator) Allocator.Error![:0]u8 {
    return std.fmt.allocPrintSentinel(allocator, "{}", .{p}, 0);
}
...
pub fn format(self: Path, writer: *std.io.Writer, comptime f: []const u8) std.io.Writer.Error!void {
    if (f.len == 1) {
        ...
        const sub_f = switch (f[0]) {
            'q' => "",
            '\'' => "\'",
            else => @compileError("unsupported format string: " ++ f),
        };
        if (self.root_dir.path) |p| {
            try stringEscape(writer, p, sub_f);
            if (self.sub_path.len > 0) try stringEscape(writer, fs.path.sep_str, sub_f);
        }
        if (self.sub_path.len > 0) {
            try stringEscape(writer, self.sub_path, sub_f);
        }
        return;
    }
    if (f.len > 0)
